Shader "PathTracing/StandardGlass"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            [shader("anyhit")]
            void AnyHitGlass(inout RayPayload payload : SV_RayPayload, AttributeData attribs : SV_IntersectionAttributes)
            {
                //payload.albedo = float3(0.0, 0.0, 1.0);
            }

            [shader("closesthit")]
            void ClosestHitGlass(inout RayPayload payload : SV_RayPayload, AttributeData attribs : SV_IntersectionAttributes)
            {
                //payload.albedo = float3(1.0, 0.0, 0.0);
                //return;
                ...
                payload.albedo                  = float3(0.6,0.7,0.8);//albedo;
